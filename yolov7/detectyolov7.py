...
def detect():
    ...
    if not opt.norwayconf_thres:
        opt.norwayconf_thres = opt.conf_thres
    if not opt.imgsznorway:
        opt.imgsznorway = imgsz
    ...
    with open(csv_file_path, mode='w', newline='') as csvfile:
        ...
        half = device.type != 'cpu'  # half precision only supported on CUDA => how about set     half to   False
        predict(weights, imgsz, device, trace, half, source, csv_writer, type='other')
        predict(weights, opt.imgsznorway, device, trace, half, source, csv_writer, type='norway')

    print(f'Saved to {csv_file_path}')

def predict(weights, imgsz, device, trace, half, source, csv_writer, type='other'):
    ...
    for path, img, im0s, origsize in dataset:
        ...
        # Apply NMS
        if type == 'norway':
            pred = non_max_suppression(pred, opt.norwayconf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms)
        else:
