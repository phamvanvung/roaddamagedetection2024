...
def filterForLargeImage(f):
    with Image.open(f) as img:
        width, height = img.size
        if width > 1824 and height > 1824:
            return True
        else:
            return False

class LoadNorwayImages:  # for inference
    def __init__(self, path, img_size=960, stride=32):
        p = str(Path(path).absolute())  # os-agnostic absolute path
        if '*' in p:
            files = sorted(glob.glob(p, recursive=True))  # glob
        elif os.path.isdir(p):
        ...
        # filter for norway
        files = [f for f in files if filterForLargeImage(f)]
    ...
    def __next__(self):
        ...
        img0 = cv2.imread(path)  # BGR
        # crop it
        origsize = img0.shape[0:2] # height, width
        img0 = img0[-1824:, :1824]
        #print(f'image {self.count}/{self.nf} {path}: ', end='')
        ...
        return path, img, img0.shape, origsize
...
class LoadOtherImages:  # for inference
    def __init__(self, path, img_size=640, stride=32):
        p = str(Path(path).absolute())  # os-agnostic absolute path
        if '*' in p:
            files = sorted(glob.glob(p, recursive=True))  # glob
        ...
        # filter for norway
        files = [f for f in files if not filterForLargeImage(f)]
        images = [x for x in files if x.split('.')[-1].lower() in img_formats]
        ni = len(images)
...
def letterbox(img, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    if new_shape[0] == shape[0] and new_shape[1] == shape[1]:
        return img, 1.0, (0,0)
